...
{ 
    ...
    public class BundlerForAI
    {
        ...
        public async static System.Threading.Tasks.Task BundleKeyVaultSQL()
        {
            // Usage:
            // dotnet run "C:\path\to\js-folder" "output.txt"
            string sourceFolder = @"D:\stefan.steiger\Documents\Visual Studio 2022\gitlab\AzureKeyVaultEmulator\AzureKeyVaultEmulator\Code\SQL\PG";
            string outputFile = System.IO.Path.Combine(ProjectDirectory, "bundle.txt");


            // List of files to exclude from the bundle(
            System.Collections.Generic.HashSet<string> sqlExcludedSet = new System.Collections.Generic.HashSet<string>(System.StringComparer.OrdinalIgnoreCase)
            {
                "DbAdmin.csproj",
                "appsettings.json",
                "README.md"
            ...
            FileFilterDelegate allSqlFiles = delegate (string path)
            {
                string ext = System.IO.Path.GetExtension(path);
                string name = System.IO.Path.GetFileName(path);

                // 1. Check extension (Case-insensitive for Linux support)
                bool isSQL = string.Equals(ext, ".sql", System.StringComparison.OrdinalIgnoreCase);
                if (!isSQL)
                    return false;

                if (sqlExcludedSet.Contains(name))
                    return false;
            ...
            // Get all files initially
            System.Collections.Generic.List<string> sqlFiles =
                GetFilesCustom(sourceFolder, allSqlFiles);
            // System.Collections.Generic.List<string> htmlFiles = GetFilesCustom(sourceFolder, allHtmlFiles);
            ...
            System.Globalization.CultureInfo culture = (System.Globalization.CultureInfo)System.Globalization.CultureInfo.InvariantCulture.Clone();
            culture.NumberFormat.NumberGroupSeparator = "'";
            System.Console.Write($"Bundled {sqlFiles.Count} ({bundleCotnent.Length.ToString("N0", culture)} bytes) JS files into: ");
            System.Console.WriteLine(outputFile);
        ...
        public async static System.Threading.Tasks.Task BundleProtocolsAdBrowser()
        {
            // Usage:
            // dotnet run "C:\path\to\js-folder" "output.txt"
            string sourceFolder = @"D:\stefan.steiger\Documents\Visual Studio 2022\gitlab\ad_browser\ProtocolsAdBrowser";
            string outputFile = System.IO.Path.Combine(ProjectDirectory, "bundle.txt");
